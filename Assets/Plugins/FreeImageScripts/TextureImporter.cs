...
namespace AsyncTextureImport
{
    ...
    public class TextureImporter
    {
        ...
        /// /// <param name="mipLevels">(optional) Number of mip levels. The default is -1 (auto)</param>
        /// <returns></returns>
        public async Task<Texture2D> ImportTextureAsync(byte[] textureData, FREE_IMAGE_FORMAT format, int mipLevels = -1) // changed IEnumerator to async task by Marius Rubo, 2022
        {
            Task<RawTextureData> task = Task.Run(() => { return ImportTextureFromMemory(textureData, format, mipLevels); });
            await task;
            Texture2D tex = await CreateTextureAsync(task.Result);

            //RawTextureData rtd = await ImportTextureFromMemoryAsync(textureData, format, mipLevels); // damn, still has that stuttering at the beginning
            //Texture2D tex = await CreateTextureAsync(rtd);
            await Task.Yield();
            return tex;
        }
        ...
        /*
        private async Task<RawTextureData> ImportTextureFromMemoryAsync(byte[] textureBytes, FREE_IMAGE_FORMAT format, int mipLevels)
        {
            if (textureBytes == null || textureBytes.Length == 0)
            {
                Debug.LogError("Empty texture data");
        ...
        */
        ...
        async Task<Texture2D> CreateTextureAsync(RawTextureData texData)
        {
            if (texData == null)
                return null;
